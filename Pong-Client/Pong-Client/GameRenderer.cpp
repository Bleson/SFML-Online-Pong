...
namespace Pong{
	GameRenderer::GameRenderer(sf::Vector2u windowSize, sf::String windowTitle, GameSimulation* game) :
		...
		, window(sf::VideoMode(windowSize.x, windowSize.y), "Pong - Client")
	{
		if (game == nullptr)
		{
	...
	void GameRenderer::Draw()
	{
		...
		if (gameToRender->GetWinner() != -1)
		{
			...
			txt.setFillColor(sf::Color::White);
			txt.setOutlineColor(sf::Color::White);
			txt.setPosition(origoPosition + sf::Vector2f(-150, -135));
			txt.setString((gameToRender->GetWinner() == gameToRender->GetPlayerNumber())? winText : loseText);
