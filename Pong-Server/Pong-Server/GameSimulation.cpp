...
namespace Pong{
	GameSimulation::GameSimulation(int playerCount) :
		ball(sf::Vector2f(5.f, 5.f), RandomBallVelocity())
	{
		playerInputs.empty();
		players.empty();
		walls.empty();
	...
	void GameSimulation::UpdateInput(int player, int movementInput)
	{
		...
		//std::cout << "Updated player " << player << 
		//	" input to value " << movementInput << "\n";
	}
	...
	sf::Packet GameSimulation::GetSnapshot()
	{
		...
		{
			//std::cout << "Player position when sending :" << var.box.y << "\n";
			packet << var.box.y;
		}
		return packet;
	}

	sf::Packet GameSimulation::GetNewRoundPacket(){
		sf::Packet packet;
		packet << static_cast<sf::Int32>(ServerMessage::RoundEnd)
			<< ball.box.x << ball.box.y
			<< ball.box.vx << ball.box.vy;
	...
	void GameSimulation::CheckForScore()
	{
		...
		if (room == nullptr)
		{
			std::cout << "Room pointer was null \n";
			return;
		}
		room->SendToAll(GetNewRoundPacket());
	}

	bool GameSimulation::CheckForWinner(){
